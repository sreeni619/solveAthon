...
//   var girl_vs_boy;
  ...
  d3.json("assets/data/states.json", function(json) {
    all_districts
    .data(json.features)
    .enter().append("path")
...
//   });
  ...
  function quantize(d) {
    district_data = data[d.properties['state_code']];
    console.log(d.properties['state_code']);
    console.log(district_data);
    // color = "q" + M ath.min(7, ~~(district_data[0]count / 20)) + "-9";
    color = "q3-9";
    return color;
  }
  ...
  function mouseover(d) {
   ...
   d3.select("#infoname").text(d.properties['state_name']);
